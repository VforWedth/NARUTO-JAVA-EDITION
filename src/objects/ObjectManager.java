...
public class ObjectManager {
	...
	public void checkSpikeTouched(Player p) {
		for(Spike s : currentLevel.getSpikes())
			if(s.getHitbox().intersects(p.getPlayerBox()))
				p.kill();
	}
	...
	private void loadImgs() {
		...
		potionImgs = new BufferedImage [2][8];
		
		for (int j = 0; j < potionImgs.length; j++)
			for(int i = 0; i < potionImgs[j].length;i++)
				potionImgs [j][i] = potionSprite.getSubimage(48 * i,48 * j, 48, 48); 
...
//			grassImgs[i] = grassTemp.getSubimage(32 * i, 0, 32, 32);
	...
	private void updateProjectiles(int[][] lvlData, Player player) {
		for (Projectile p : projectiles)
			if (p.isActive()) {
				p.updatePos();
				if (p.getHitbox().intersects(player.getPlayerBox())) {
	...
	private boolean isPlayerInRange(Cannon c, Player player) {
		int absValue = (int) Math.abs(player.getPlayerBox().x - c.getHitbox().x);
		return absValue <= Game.TILES_SIZE * 5;
	}
	
	private boolean isPlayerInfrontOfCannon(Cannon c, Player player) {
		if (c.getObjType() == CANNON_LEFT) {
			if (c.getHitbox().x > player.getPlayerBox().x)
				return true;

		} else if (c.getHitbox().x < player.getPlayerBox().x)
			return true;
		return false;
	}
				...
				if (c.getTileY() == player.getTileY())
					if (isPlayerInRange(c, player))
						if (isPlayerInfrontOfCannon(c, player))
							if (CanCannonSeePlayer(lvlData, player.getPlayerBox(), c.getHitbox(), c.getTileY()))
								c.setAnimation(true);
		...
		for(GameContainer gc : containers)
			if(gc.isActive()) {
				...
				if(gc.objType == BARREL) 
					type = 1;
				g.drawImage(containerImgs[type][gc.getAniIndex()],(int)(gc.getHitbox().x - gc.getxDrawOffset() - xLvlOffset),(int)(gc.getHitbox().y - gc.getyDrawOffset()),
